...
bot.on("message", async message => {
    ...
    Guilds.findOne({guildID: message.guild.id}, async(err, data) => {
        ...
        if(message.content.startsWith(`${data.prefix}invite`)) {
            let embed__invite = new Discord.MessageEmbed()
            .setTitle(`DiorBot | Приглашение бота!`)
            .setDescription('\`DiorBot - разнофункциональный бот, имеющий много разных и интересных функций и систем! Бот предназначен для: развлечений, модерирования, проведения мероприятий и т.д. Разработчик и основатель бота настоятельно рекомендуют выдавать права нашему боту на вашем сервере, так как именно благодаря этому праву - некоторые функции бота работают исправно.\`')
